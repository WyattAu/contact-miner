...
impl EmailVerifier {
    ...
    pub async fn verify(&self, email: &str) -> bool {
        ...
        // MX record check
        let domain = canonical.split('@').next_back().unwrap_or("");
        let has_mx = self.check_mx_record(domain).await;

        // Cache result with timestamp
        self.mx_cache
            .insert(canonical, (has_mx, std::time::Instant::now()));
    ...
    pub async fn smtp_verify(&self, email: &str) -> Option<bool> {
        ...
        self.smtp_cache
            .insert(canonical, (result, std::time::Instant::now()));
            ...
            Ok(o) => {
                ...
                if let Ok(result) = serde_json::from_str::<serde_json::Value>(&stdout) {
                    ...
                    if confidence >= 0.6 {
                        ...
                        warn!(
                            "SMTP verify {} low confidence: {:?} ({})",
                            email, deliverable, reason
                        );
                    }
                    return deliverable;
                }
    ...
    pub fn validate_phone(phone: &str) -> bool {
        ...
        if invalid.iter().any(|&p| digits == p) {
            return false;
        }
        // Reject strictly ascending sequential numbers (1234567, 2345678, ...)
        if digits.len() >= 7 {
            let sequential = digits.as_bytes().windows(2).all(|w| w[1] == w[0] + 1);
            if sequential {
                return false;
            }
        }
    ...
    pub fn normalize_followers(raw: &str) -> Option<u64> {
        ...
        if raw.ends_with('M') || raw.ends_with('m') {
            if let Ok(n) = raw.trim_end_matches(['M', 'm']).parse::<f64>() {
                return Some((n * 1_000_000.0) as u64);
            }
        }
        if raw.ends_with('K') || raw.ends_with('k') {
            if let Ok(n) = raw.trim_end_matches(['K', 'k']).parse::<f64>() {
    ...
    pub fn analyze_bio_intent(bio: &str) -> BioIntent {
        ...
        let has_business_email = bio_lower.contains("business")
            || bio_lower.contains("collab")
            || bio_lower.contains("partnership")
            || bio_lower.contains("inquiry")
            || bio_lower.contains("contact");
        let has_dm_prompt = bio_lower.contains("dm")
            || bio_lower.contains("direct message")
            || bio_lower.contains("message me");
        let has_link = bio_lower.contains("link")
            || bio_lower.contains("linktr.ee")
            || bio_lower.contains("beacons.ai")
            || bio_lower.contains("bit.ly");
        let has_email_pattern = bio_lower.contains("@")
            && (bio_lower.contains("gmail")
                || bio_lower.contains("outlook")
                || bio_lower.contains("yahoo"));
        BioIntent {
            has_business_email,
            has_dm_prompt,
            has_link,
            has_email_pattern,
            confidence: if has_business_email || has_email_pattern {
                0.8
            } else if has_dm_prompt || has_link {
                0.5
            } else {
                0.2
            },
    ...
    pub fn calculate_score(
        ...
        has_bio: bool,
        has_website: bool,
        bio_intent: &BioIntent,
    ) -> f64 {
        let mut score = 0.0;
        score += match follower_count.unwrap_or(0) {
            0..=1000 => 5.0,
            1001..=10000 => 15.0,
            10001..=100000 => 25.0,
            100001..=1000000 => 35.0,
        ...
        if bio_intent.has_business_email {
            score += 5.0;
        }
        if bio_intent.has_dm_prompt {
            score += 3.0;
        }
        if bio_intent.has_link {
            score += 2.0;
        }
        score.min(100.0)
    }
...
mod tests {
    ...
    fn test_canonicalize_email() {
        ...
        assert_eq!(
            canonicalize_email("test..email@domain.com"),
    ...
    fn test_normalize_followers() {
        ...
        assert_eq!(
            EmailVerifier::normalize_followers("1,234,567"),
            Some(1_234_567)
        );
    }
